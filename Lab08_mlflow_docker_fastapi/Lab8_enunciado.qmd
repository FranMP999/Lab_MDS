---
title: "**Laboratorio 8: Ready, Set, Deploy! \U0001F469‍\U0001F680\U0001F468‍\U0001F680**"
jupyter: lab_env
---
...
# Initiate the parent run and call the hyperparameter tuning child run logic
    ...
    with mlflow.start_run(
        ...
        best_model.fit(X, y)
        with open('serialized_model.pkl', 'wb') as file:
...
- **¿Cómo se diferencia Docker de una máquina virtual (VM)?**
    ...
    Las máquinas virtuales comprenden una copia completa del sistema operativo 
    lo cual puede consumir una cantidad significativa de espacio en disco y puede 
    requerir varios minutos para poder arrancar, mientras que los contenedores 
    utilizan el sistema operativo del host y comparten sus recursos, por lo que 
    inician en cuestión de segundos y permiten una escabilidad y despliegue mucho 
    más rápidos, además al ser gestionados por el motor de contenedores y estar 
    construidos a partir de imágenes ligeras y definidas por archivos de 
    configuración son más sencillas y eficientes.

- **¿Cuál es la diferencia entre usar Docker y ejecutar la aplicación directamente en el sistema local?**

  
    Docker nos permite desarrollar el proyecto en contenedores, que a diferencia de la máquina local
    son ligerosa, permitiendo ejecutar múltiples instancias en un solo sitema sin una sobrecarga significativa,
    son portables, por lo que pueden moverse fácilmente entre diferentes entornos y lo más importante es que 
    proporcionan un entrono aislado para cada aplicación, evitando conflictos entre diferentes aplicaciones y
    mejorando la seguridad.
- **¿Cómo asegura Docker la consistencia entre diferentes entornos de desarrollo y producción?**

  
    Docker encapsula las aplicaciones y y sus dependencias en contenedores, 
    cada contenedor incluye su propio conjunto de bibliotecas y componentes 
    necesarios para la aplicación, garantizando que se ejecuten de manera aislada
    y consistente, independientemente del entorno subyacente.
- **¿Cómo se gestionan los volúmenes en Docker para la persistencia de datos?**

  
    Los volúmenes en Docker se gestionan a través de la modificación
    del comando de ejecución del contenedor, montando el volumen que enlace una carpeta del 
    host con la carpeta del contenedor con la sintáxis `-v <directorio_local>:<directorio_container>`
    como parámetro opcional en el comando.

  
- **¿Qué son Dockerfile y docker-compose.yml, y cuál es su propósito?**

  
    Dockerfile es el archivo que define cómo se construye la imagen, cada
    instrucción en un Dockerfile  es una palabra reservada que se utiliza 
    durante el proceso de construcción de la imagen. Con el propósito de centralizar
    el proceso de creación de imagen en un archivo único para mayor claridad, 
    posibilidad de incluir un sistema de versiones y la automaoización del 
    proceso de construcción. Este archivo permite a un desarrollador identificar 
    rápidamente todas las dependencias y configuraciones necesarias para 
    ejecutar un proyecto.

    El archivo docker-compose.yml  es el núcleo de Docker Compose, donde se define la 
    configuración de los servicios que componen una aplicación Docker multi-
    contenedor, utiliza el formato YAML para definir los servicios, redes y volúmentes.
